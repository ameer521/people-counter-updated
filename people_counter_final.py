...
ipaddress =(input("Enter The IP Address of NVR/DVR\n"))
portno = input("Enter the Port Number\n")
username = input("Enter the username of NVR/DVR\n")
password = input("Enter The password of NVR/DVR\n")
...
if not args.get("input", False):  # if the input video is not available o
	...
	global vs
	vs = cv2.VideoCapture('rtsp://admin:audreytech*1@192.168.1.245:554')
	vs = cv2.VideoCapture(protocol+'://'+username+':'+password+'@'+ipaddress+':'+portno)
	...
	while True:
		global frame
		_,frame = vs.read()  # the vs contain the video by videocapture
